...
Vous y retrouverez ainsi, la température, l'humidité, la vitesse du vent , les précipitations et l'heure du couché et du levé du soleil.
...
def date_heure():
 ...
 heure_format = now.strftime('%Hh%M')  # Format HH h MM
 return f"Nous sommes le {date_format}, et il est : {heure_format}"
print(f" {date_heure()}")
```
...
if response.status_code == 200:
 ...
 Humidité=[(humidity[0][heure()],"{:.3f}".format(np.mean(humidity[0]))),(np.max(humidity[1]),"{:.3f}".format(np.mean(humidity[1]))),(np.max(humidity[2]),"{:.3f}".format(np.mean(humidity[2]))),(np.max(humidity[3]),"{:.3f}".format(np.mean(humidity[3])))]
 Vent=[(wind[0][heure()],"{:.3f}".format(np.mean(wind[1]))),(np.max(wind[1]),np.min(wind[1])),(np.max(wind[2]),np.min(wind[2])),(np.max(wind[3]),np.min(wind[3]))]
 Pluie=[(precipitation[0][heure()],"{:.3f}".format(np.mean(precipitation[0]))),(np.max(precipitation[1]),"{:.3f}".format(np.mean(precipitation[1]))),(np.max(precipitation[2]),np.mean(precipitation[2])),(np.max(precipitation[3]),"{:.3f}".format(np.mean(precipitation[3])))]
 ...
 Tab=pd.DataFrame(tab,index=['Température','Humidité','Vitesse du vent','Précipitation','Sunrise/Sunset'],
