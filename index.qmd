...
if response.status_code == 200:
 ...
 j=correspondance.get(jour()) # jour actuel
 #Cette fonction renvoie une liste des 4 prochains jours qui servira pour l'affichage du tableau 
 def jours():
   if (j==1):
 ...
 temp=list(chunked(temp, 24)) #on utilise chunked pour faire 4 listes de 24 éléments qui seront les données de temmpératures par jours
 ...
 #On regarde a quelle heure de la journée on se situt pour 
 def heure():
    for i in range(24):
       time[0][i]=time[0][i+1]
 ...
 Humidité=[(humidity[0][heure()],"{:.2f}".format(np.mean(humidity[0]))),(np.max(humidity[1]),"{:.2f}".format(np.mean(humidity[1]))),(np.max(humidity[2]),"{:.2f}".format(np.mean(humidity[2]))),(np.max(humidity[3]),"{:.2f}".format(np.mean(humidity[3])))]
 Vent=[(wind[0][heure()],"{:.2f}".format(np.mean(wind[1]))),(np.max(wind[1]),np.min(wind[1])),(np.max(wind[2]),np.min(wind[2])),(np.max(wind[3]),np.min(wind[3]))]
 Pluie=[(precipitation[0][heure()],"{:.2f}".format(np.mean(precipitation[0]))),(np.max(precipitation[1]),"{:.2f}".format(np.mean(precipitation[1]))),(np.max(precipitation[2]),np.mean(precipitation[2])),(np.max(precipitation[3]),"{:.2f}".format(np.mean(precipitation[3])))]
 Soleil=[(sunrise[0],sunset[0]),(sunrise[1],sunset[1]),(sunrise[2],sunset[2]),(sunrise[3],sunset[3])]
 #On crée le tableau qui sera l'affichage
 ...
 Tab=pd.DataFrame(tab,index=['Température','Humidité','Vitesse du vent','Précipitations','Sunrise/Sunset'],
